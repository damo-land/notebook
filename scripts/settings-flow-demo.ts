...
// double Enter, the provider toggle switches the model-list source, the save
// plan holds only what changed (vault strictly before llm), and Esc is
// swallowed on first run but closes the settings view otherwise.
...

{
  ...
  const first = wizardConfirm(w, { vaultPath: "/Users/me/Stash", llm });
  assert.deepStrictEqual(first.action, { cmd: "set_vault_dir", path: "/Users/me/Stash" });
  assert.strictEqual(first.state.step, "ai");
  assert.strictEqual(first.state.done, false);

  // Second Enter: the AI step is preselected claude / default model, so the
  // untouched confirm saves exactly those defaults and finishes the wizard.
  const second = wizardConfirm(first.state, { vaultPath: "/Users/me/Stash", llm });
  assert.deepStrictEqual(second.action, {
    cmd: "set_llm_config",
    provider: "claude",
...

{
  ...
  // Nothing changed: nothing to save (Enter just closes).
  assert.deepStrictEqual(
    savePlan({ ...initial, vaultPath: "/v/old", llm: llmUnchanged }),
    []
  );

  // Vault only.
  assert.deepStrictEqual(savePlan({ ...initial, vaultPath: "/v/new", llm: llmUnchanged }), [
    { cmd: "set_vault_dir", path: "/v/new" },
  ]);

  // LLM only (model change).
  const llmChanged = withModel(llmUnchanged, "claude-opus-5");
  assert.deepStrictEqual(savePlan({ ...initial, vaultPath: "/v/old", llm: llmChanged }), [
    { cmd: "set_llm_config", provider: "claude", model: "claude-opus-5" },
  ]);

  // Both changed: vault first, llm second — the caller awaits each action in
  // order, so set_vault_dir and set_llm_config never run concurrently.
  assert.deepStrictEqual(savePlan({ ...initial, vaultPath: "/v/new", llm: llmChanged }), [
    { cmd: "set_vault_dir", path: "/v/new" },
    { cmd: "set_llm_config", provider: "claude", model: "claude-opus-5" },
  ]);
  ...
  assert.deepStrictEqual(
    savePlan({ initialVaultPath: "/v/old", initialLlm: null, vaultPath: "/v/old", llm: llmUnchanged }),
...

{
  ...
  assert.deepStrictEqual(fieldOrder("wizard", "ai"), ["provider", "model"]);
  assert.deepStrictEqual(fieldOrder("settings", "ai"), ["vault", "provider", "model"]);

  const order = fieldOrder("settings", "ai");
  assert.strictEqual(nextField(order, "vault", 1), "provider");
  assert.strictEqual(nextField(order, "model", 1), "vault"); // wraps forward
  assert.strictEqual(nextField(order, "vault", -1), "model"); // wraps back
